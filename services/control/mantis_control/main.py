...
app = FastAPI(title="Mantis-DNS Control Plane", version="0.1.0")
...
def _check_production_secrets() -> None:
    ...
    if os.environ.get("MANTIS_ENV", "").lower() != "production":
        return
    errors: list[str] = []
    jwt_secret = os.environ.get("MANTIS_JWT_SECRET", _JWT_DEV_SECRET)
    ...
    if not os.environ.get("MANTIS_WEBHOOK_SECRET_KEY"):
        errors.append("MANTIS_WEBHOOK_SECRET_KEY is not set")
    internal_token = os.environ.get("MANTIS_INTERNAL_TOKEN", _INTERNAL_TOKEN_DEV_DEFAULT)
    ...
    if not os.environ.get("MANTIS_SERVICE_TOKEN"):
...
def on_startup() -> None:
    ...
    try:
        ...
        if db.query(User).count() == 0:
            admin_email = os.environ.get("ADMIN_EMAIL", "admin@mantis.local")
            admin_password = os.environ.get("ADMIN_PASSWORD", "change-me-now")
