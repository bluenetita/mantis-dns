...
**Daemon liveness (`dhcp_daemon_heartbeats`)**: neither daemon had any liveness signal reaching the control plane before this — a crashed or bootlooping process (e.g. a leftover `kea-dhcp4` still holding `:67` from before the migration) showed up nowhere except `journalctl`; the Status tab's lease/utilisation numbers just stopped updating silently, with nothing telling anyone *why*. Each instance now upserts its own row (`instance_id` — a fresh UUID generated at its own startup, not persisted anywhere else; `family`; `hostname`, best-effort via `/proc/sys/kernel/hostname`; `started_at`; `last_seen_at`) on the same tick as its existing config-refresh loop (`scope_refresh_interval_s`, 10s by default), and prunes any other same-family row that's gone stale (>5 minutes) on every tick — self-cleaning, no separate sweep job. `GET /api/v1/dhcp/health` (`require_role("operator")`, since hostnames/topology aren't tenant-scoped data) flags a row `stale` once its `last_seen_at` is more than 30s old (3x the heartbeat cadence); a daemon down longer than the 5-minute prune window has no row at all rather than one stuck showing `stale` forever. Surfaced on the Status tab as a green/red badge per instance.
